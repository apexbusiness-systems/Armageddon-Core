...
| Render deployment | Restored 2026-07-09 — unified single-container | `render.yaml` defines the unified Web Service co-running the API server and worker via `start.sh` (free tier compatible). |
...
| --- | --- | --- |
| 2026-07-12 | Resolved P0 deployability blocker: copied `bundle-workflows.mjs` into both production Dockerfiles before execution. Hardened SSRF validation against CGNAT, IPv4-mapped/compatible IPv6 formats, URL userinfo, and non-global ranges. Added compile and syntax contract checks for Docker builds, lint barriers for Temporal barrel imports, and fail-closed provider factories to prevent silent simulation fallback for certified-live runs. Node version pinned to 20 across CI and Docker. | `scripts/check-docker-build-contract.mjs`, `packages/shared/src/omniport.ts`, `packages/core/src/providers/index.ts`, `packages/core/src/temporal/workflows.ts`, `.github/workflows/production-readiness.yml`, `omni-recall/2026-07-12-p0-rescue-docker-ssrf-temporal.md`. |
