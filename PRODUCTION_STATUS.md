...
| OmniPort connector (Level 8 / Kinetic Moat) | Defined, provisioning still operator-gated | `packages/core/src/api-server.ts` → `handleOmniPort*`; `packages/shared/src/omniport.ts`; `docker-compose.moat.cloud.yml`. `OMNIPORT_ENABLED` is unset (safe default: routes return 503) on the live Render deployment above. **Where `api-server.ts` runs in production is now committed and live** (see the Render deployment row — this superseded the prior "not committed to this repository" note). **Still not usable for a real live-fire run regardless of provisioning**: `packages/core/src/worker.ts` refuses to start unless `SIM_MODE=true` (protected invariant, `CLAUDE.md` Invariant 10) — confirmed live-verified: the deployed service reports `simMode:true`. See `feature_registry.md` "OmniPort Connector" domain. |
| Certification-run execution pipeline | **Live-verified 2026-07-22 — end-to-end** | A real authenticated Level 7 run against production reached a terminal `passed` state with all 5 batteries (B10–B14) executing and reporting telemetry, on the Render deployment above. See "Certification pipeline resolution" section below — this closes the P1 that was open earlier the same day. |
| Attestation public-key endpoint | **Split status, both halves live-verified 2026-07-22** | On the public marketing domain (`https://armageddontest.icu/api/attestation/pubkey`) this still returns the static SPA shell, not JSON — that route is edge/static-export-only and never reaches `handleAttestationPubkey`; unchanged since the 2026-07-04 correction. On the Render execution engine directly (`https://armageddon-exec-api.onrender.com/api/attestation/pubkey`), `curl` confirms a real signed response (`{"spec":"armageddon-attestation/1.0","algorithm":"ed25519","keyId":"b98b4b67994fccb5","publicKey":"...","issuedAt":"..."}`) — `handleAttestationPubkey` in `packages/core/src/api-server.ts` is live and correctly configured with `ARMAGEDDON_ATTESTATION_SEED`. Operators/clients needing this endpoint must call the Render URL directly today; it is not (yet) proxied through the public domain. |

---

## ✅ RESOLVED (was P1, live-verified 2026-07-22): certification runs stuck at `pending`

**Originally**: a real authenticated certified-tier run created against production (`POST /api/run` ✅) never reached a terminal state — it stayed `pending` for 300s with zero `armageddon_events`, and `/api/omniport/health` reported `temporalConnected:false`. Root cause and full evidence: [`docs/audits/PRODUCTION_RUN_DISPATCH_STUCK_2026-07-22.md`](docs/audits/PRODUCTION_RUN_DISPATCH_STUCK_2026-07-22.md).

**Resolution, deployed and live-verified the same day**: PR #206 shipped Wake-on-Enqueue (edge) and Active-Run Self-Sustain (executor), closing the free-tier idle/cold-start gap without any paid plan or new dependency; PR #207 fixed a related verdict-integrity defect the investigation surfaced. Both merged and confirmed live on `main` (Render deploy `e833cd5`, Cloudflare `sourceCommit: e833cd5`, both cross-checked directly against their respective APIs — not assumed). A certification-run execution pipeline row above documents an end-to-end `passed` run on this deployed code. No certificate is issued from a fabricated or incomplete run — only from a run that reaches a real terminal state through this pipeline, per this file's standing evidence rule.
