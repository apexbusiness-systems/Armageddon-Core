...
function promptConfirmation(query: string): Promise<string> {
...
program
  ...
  .action(async (options) => {
    ...
    if (mode === 'simulation') {
        ...
        // DESTRUCTION-GRADE GATING
        console.warn('\n⚠️  DESTRUCTION MODE ENABLED ⚠️');
        console.warn('You are responsible for:');
        console.warn('  - Ensuring this is a non-production environment');
        console.warn('  - Having authorization to run destructive tests');
        console.warn('  - Restoring the environment after testing');

        const confirmation = await promptConfirmation('Type "I ACCEPT RESPONSIBILITY" to continue: ');
        if (confirmation !== 'I ACCEPT RESPONSIBILITY') {
            console.error('Destruction mode cancelled by user.');
            process.exit(1);
        }
    }
    ...
    try {
        if (options.worker) {
            console.log('[CLI] Starting local worker (using SQLite storage)...');
            // Ensure we use local storage logic in worker initialization if needed
            // The Reporter logic already handles fallback to SQLite if Supabase envs are missing
  ...
  .action(async (options) => {
    try {
        ...
        if (report.rerun_command) {
            console.log(`[VERIFY] Rerun Command: ${report.rerun_command}`);
        }
    } catch (err) {
