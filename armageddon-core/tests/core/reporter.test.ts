...
describe('createReporter factory', () => {
  ...
  it('creates a new instance on each call (no caching)', async () => {
    // The source implementation calls `new SupabaseReporter(runId)` each time.
    const { createReporter } = await import('../../src/core/reporter');
    const r1 = createReporter('run-no-cache-001');
    const r2 = createReporter('run-no-cache-001');
    // createReporter is a plain factory — not a cache — so instances differ
