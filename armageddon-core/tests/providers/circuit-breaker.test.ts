...
describe('CircuitBreaker', () => {
  ...
  it('DEFAULT_CIRCUIT_CONFIG has expected shape', () => {
    expect(DEFAULT_CIRCUIT_CONFIG).toMatchObject({
      maxCostUSD: expect.any(Number),
      maxTokensPerRun: expect.any(Number),
      maxRequestsPerMinute: expect.any(Number),
      maxConsecutiveErrors: expect.any(Number),
...
describe('CircuitBreakerRegistry', () => {
  ...
  it('resetAll resets all registered breakers', () => {
    const reg = CircuitBreakerRegistry.getInstance();
    const b = reg.getOrCreate('provider-reset-test');
    b.recordError();
    b.recordError();
    b.recordError();
