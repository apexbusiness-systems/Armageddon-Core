...
describe('CircuitBreaker', () => {
    ...
    it('should transition from HALF_OPEN back to OPEN on error', () => {
        vi.useFakeTimers();
        ...
        // Error in HALF_OPEN should go back to OPEN
        breaker.recordError();
        expect(breaker.getState()).toBe('OPEN');
        expect(breaker.canProceed()).toBe(false);
    ...
    it('should reset consecutive error count after success', () => {
        // maxConsecutiveErrors = 2
        breaker.recordError();  // consecutiveErrors = 1
        expect(breaker.getState()).toBe('CLOSED');

        // Success should reset counter
        breaker.recordSuccess(1, 1, 10);

        // These errors should not trip (counter was reset)
        breaker.recordError();
        expect(breaker.getState()).toBe('CLOSED');
        breaker.recordError();
        expect(breaker.getState()).toBe('OPEN');  // Now it trips
    });

    it('should allow exactly maxTokensPerRun', () => {
        // maxTokensPerRun = 1000
        breaker.recordSuccess(500, 500, 100);  // Exactly 1000 tokens
        // At exactly the limit, we should NOT allow more requests (Zero Tolerance)
        expect(breaker.canProceed()).toBe(false);
        // State might technically be OPEN if canProceed checks strict >=.
        // Let's check implementation: if (tokens >= max) trip().
        // So yes, it should be OPEN.
        expect(breaker.getState()).toBe('OPEN');
    });

    it('should trip at maxTokensPerRun + 1', () => {
        breaker.recordSuccess(500, 501, 100);  // 1001 tokens
        expect(breaker.canProceed()).toBe(false);  // Should trip
        expect(breaker.getState()).toBe('OPEN');
    });

    it('should allow exactly maxCostUSD', () => {
        // maxCostUSD = 1.0
        // 100k input tokens = $1.0
        breaker.recordSuccess(100000, 0, 100);
        expect(breaker.canProceed()).toBe(false);  // Should trip at exact limit
        expect(breaker.getState()).toBe('OPEN');
    });
    ...
    it('should reset rate limit after 60 seconds', () => {
        ...
        expect(breaker.canProceed()).toBe(false);
        expect(breaker.getState()).toBe('CLOSED');  // Not tripped, just rate limited
        ...
        expect(breaker.canProceed()).toBe(true);
        breaker.recordSuccess(1, 1, 10);  // Should succeed
        expect(breaker.getState()).toBe('CLOSED');
    ...
    it('should handle negative token counts gracefully', () => {
        // Should not crash or produce negative costs
        breaker.recordSuccess(-100, 100, 10);
        const metrics = breaker.getMetrics();
    ...
    it('should handle zero tokens without crashing', () => {
        breaker.recordSuccess(0, 0, 10);
        expect(breaker.canProceed()).toBe(true);
    ...
    it('should handle negative latency gracefully', () => {
        breaker.recordSuccess(100, 100, -50);  // Negative latency shouldn't break
        const metrics = breaker.getMetrics();
...
describe('CircuitBreakerRegistry', () => {
    ...
    it('should enforce global token limit across all providers', () => {
        // Configure providers with high limits so they don't trip locally
        const p1 = registry.getOrCreate('provider-1', { maxTokensPerRun: 1000000 });
        const p2 = registry.getOrCreate('provider-2', { maxTokensPerRun: 1000000 });
        const global = registry.getGlobal();

        // Global limit is 500,000 tokens
        // Provider 1: 200k tokens (OK)
        p1.recordSuccess(200000, 0, 100);
        expect(global.canProceed()).toBe(true);

        // Provider 2: 301k tokens (Total 501k > 500k limit)
        p2.recordSuccess(301000, 0, 100);

        expect(global.canProceed()).toBe(false);  // Global limit reached
        expect(global.getState()).toBe('OPEN');
    });
});
