...
describe('POST /api/gatekeeper', () => {
    ...
    beforeEach(() => {
        ...
        delete process.env.ARMAGEDDON_ADMIN_EMAIL;
    });
    ...
    it('should return eligible: true for ARMAGEDDON_ADMIN_EMAIL alias when ADMIN_EMAIL is absent', async () => {
        delete process.env.ADMIN_EMAIL;
        process.env.ARMAGEDDON_ADMIN_EMAIL = 'alias-admin@example.com';
        (authenticateRequest as any).mockResolvedValueOnce({
            user: { email: 'alias-admin@example.com', id: 'admin-2' },
            supabase: mockSupabase,
        });

        const req = new NextRequest('http://localhost:3000/api/gatekeeper', {
            method: 'POST',
            headers: { Authorization: 'Bearer alias-admin-token' },
        });

        const res = await POST(req);
        const data = await res.json();

        expect(data).toEqual({ eligible: true, tier: 'certified', reason: 'ADMIN_OVERRIDE' });
    });

    it('should return eligible: false for lookalike admin email', async () => {
        (authenticateRequest as any).mockResolvedValueOnce({
            user: { email: 'prefix-admin@example.com.evil', id: 'attacker-1' },
            supabase: mockSupabase,
        });

        const req = new NextRequest('http://localhost:3000/api/gatekeeper', {
            method: 'POST',
            headers: { Authorization: 'Bearer lookalike-token' },
        });
        ...
        expect(data.eligible).toBe(false);
        expect(data.reason).toBe('LEVEL_7_ACCESS_REQUIRED');
    });
