                ...
                <div className="grid lg:grid-cols-2 gap-4 mb-16">
                    {BATTERIES.map((battery, index) => (
                        <motion.div
                            ...
                            onClick={() => setExpandedId(expandedId === battery.id ? null : battery.id)}
                            onMouseEnter={() => setHoveredId(battery.id)}
                            onMouseLeave={() => setHoveredId(null)}
                            className={`
                card-panel cursor-pointer
                ${battery.godMode ? 'card-highlight' : ''}
                ${index === BATTERIES.length - 1 && BATTERIES.length % 2 === 1 ? 'lg:col-span-2 lg:max-w-[calc(50%-0.5rem)] lg:mx-auto' : ''}
              `}
                        >
                            {/* God Mode badge */}
                            {battery.godMode && (
                            ...
                            <div className={`flex items-center gap-5 p-5 ${battery.godMode ? 'pr-24' : ''}`}>
                                ...
                                {/* Expand indicator */}
                                <motion.div
                                    animate={{ rotate: expandedId === battery.id ? 180 : 0 }}
                            ...
                            <motion.div
                                ...
                                initial={{ width: 0 }}
                                animate={{ width: hoveredId === battery.id ? '100%' : 0 }}
